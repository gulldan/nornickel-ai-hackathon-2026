// Package application contains office-parser's use case: consume a
// DocumentUploaded event, extract text from the office document, and forward it
// to chunk-splitter. It depends only on small ports (object store, status
// updater, publisher) so it stays storage- and transport-agnostic. Every parser
// worker follows this same shape.
package application
...
func (p *Processor) Process(ctx context.Context, evt *commonv1.DocumentUploaded) error {
	...
	switch {
	case p.extractor.IsPlainText(evt.GetFilename(), evt.GetMimeType()):
		// Plain-text source: the object itself is the text. Don't download it at
		// all — hand chunk-splitter a reference (zero-copy claim check); on
		// hundreds of megabytes downloading here would OOM for nothing.
		parsed.TextObjectKey = evt.GetObjectKey()
		log.Info().Str("text_object_key", parsed.GetTextObjectKey()).Msg("plain text passed by reference")
	default:
		data, err := p.store.GetBytes(ctx, evt.GetObjectKey())
		if err != nil {
			return p.fail(ctx, evt, fmt.Errorf("download %s: %w", evt.GetObjectKey(), err))
		}
		result, err := p.extractor.Extract(ctx, data, evt.GetFilename(), evt.GetMimeType())
		if err != nil {
			return p.fail(ctx, evt, fmt.Errorf("extract text: %w", err))
		}
		// Пустой результат — это провал, а не «документ с 0 чанков»: иначе файл
		// молча числится проиндексированным без контента.
		if strings.TrimSpace(result.Text) == "" {
			return p.fail(ctx, evt, errors.New("extracted text is empty (no readable text in document)"))
		}
		parsed.Text = result.Text
		if len(result.Metadata) > 0 {
			parsed.Metadata = copyMetadata(result.Metadata)
		}
		if err := p.storeSidecars(ctx, evt, parsed, result.Sidecars); err != nil {
			return p.fail(ctx, evt, err)
		}
		if len(parsed.GetText()) > inlineTextMax {
			key := "parsed/" + evt.GetDocumentId() + ".txt"
			perr := p.store.Put(ctx, key, strings.NewReader(parsed.GetText()), int64(len(parsed.GetText())), "text/plain; charset=utf-8")
			if perr != nil {
				return p.fail(ctx, evt, fmt.Errorf("store parsed text: %w", perr))
			}
			parsed.Text, parsed.TextObjectKey = "", key
			log.Info().Int("chars", len(result.Text)).Str("text_object_key", key).Msg("text offloaded to object store")
		}
	}
